...
pipe() {
    log "--- Running test case: ${FUNCNAME} ---"
    run_test_case "echo hello world hello world | grep hello | wc -m\nexit\n"
...
## Extra feature #1: stderr redirection
stderr_out_redir() {
    log "--- Running test case: ${FUNCNAME} ---"
    run_test_case "ls file_that_doesnt_exists >& t\ncat t\nexit\n"
    rm -f t

    local line_array=()
    line_array+=("$(select_line "${STDOUT}" "3")")
    local corr_array=()
    corr_array+=("ls: cannot access 'file_that_doesnt_exists': No such file or directory")

    local score
    compare_lines line_array[@] corr_array[@] score
    log "${score}"
}
TEST_CASES+=("stderr_out_redir")
